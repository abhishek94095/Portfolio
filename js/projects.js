/**
 * projects.js — No-tab split panel, full-page theme toggle
 */
(function () {
    ...
    function iconHTML(key) {
        ...
        const inner = cfg.devicon
            ? `<img src="${DEVICON_BASE}${cfg.devicon}.svg" alt="${key}" class="pstack__img" onerror="this.parentElement.innerHTML='<span class=pstack__init style=color:${cfg.color}>${fallback}</span>'">`
            : `<span class="pstack__init" style="color:${cfg.color}">${fallback}</span>`;
    ...
    function renderDetail(p) {
        ...
        const metric = p.metric
            ? `<div class="pdet__metric"><span class="pdet__metric-label">${p.metric.label}:</span> ${p.metric.value}</div>`
            : '';
        ...
        return `
            <p class="pdet__desc">${p.description}</p>
    ...
    function updateList() {
        ...
        document.querySelectorAll('.plist__item').forEach((el, i) =>
            el.classList.toggle('plist__item--active', i === idx));
    }
